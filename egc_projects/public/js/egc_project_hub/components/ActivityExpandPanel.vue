<!-- Inline "drop down and showcase what's underneath" panel for one Activities-table row
     ...
     without navigating away from the list — ActivityFullPage.vue remains the place to see and
     edit everything about an Activity, reached via the "Open full detail" link this panel
     exposes).

     Reuses get_activity_detail (the same call ActivityDetail.vue/ActivityFullPage.vue make) —
     this is purely a different, inline shell around data the app already fetches and displays. A
     small "tools" strip (mirroring Procore's own per-record tool switcher) lets a Project Manager
     flip between what's underneath an Activity without the panel growing to show everything
     stacked at once; each tool's badge count is available immediately from `linkCounts` (already
     fetched for the whole table in one query — see api/hub.py's get_activities), before the
     per-Activity detail round trip even resolves.

     Restored (direct user instruction, 2026-08-31): this file existed before, was removed when
     ActivityFullPage.vue was introduced on the theory that the drawer/full page already covered
     the same ground — the user explicitly wants the inline row expand back as ITS OWN surface,
     distinct from both the side-panel drawer and the full page, for the table's row-click. -->
<script setup>
