...
notify_new_revision/send_due_date_reminders. Previously only the email side
(test_notifications_email.py) had any coverage at all.

The three event-triggered notifiers (submission received, response recorded, new revision) now
create a real Frappe ToDo via `_assign_task` (`frappe.desk.form.assign_to._add`), mirroring
`submittal_control.py`'s own ball-in-court `_assign_step` — a passive Notification Log alone was
not an actionable "task" for the submittal_manager/originator roles, which is exactly the gap
these tests guard against regressing to. `_add` is mocked rather than asserting a real ToDo row
exists, matching how `_assign_step` itself is exercised elsewhere in this app's test suite.

`send_due_date_reminders` is unrelated — still a plain in-app reminder ping via
`enqueue_create_notification` (dispatched through `frappe.enqueue(...)` onto a real Redis queue,
not synchronous), kept on `_notify` and covered by its own test below unchanged.
"""
